...
commit;
host start sqlplus sistema/sistema @C:\bd2\Crear_Triggers.sql

host pause
insert into PLA01(a) values (1);
commit;

--otro spool... drop de los triggers?
spool C:\bd2\Dropear_Triggers.sql
set pagesize 0
set linesize 300
set feedback off
set termout off
--select 'drop trigger '||table_name||'_bir'||';'
select 'alter trigger '||table_name||'_bir'||' disable;'
from   dba_tables
where  owner='SISTEMA'
and    table_name like 'PLA%'
order by table_name;
...
set feedback on
set termout on
host start sqlplus sistema/sistema @C:\bd2\Dropear_Triggers.sql
